...
export const getProjectsNextStatusId = project => {
	const currentStatus = project.STATUS_ID;
	...
	const statusMap = caesCache.data.crfp.status.reduce((map, status) => {
		...
		} else if (stage < 3 && project.FINAL_SITE_APPROVER_PERSONNEL_ID !== null) {
			map[status.ID] = statusesIndexedByName['Pending Final Site Approver Approval'];
		} else if (stage < 4 && project.OFFICE_OF_RESEARCH_PERSONNEL_ID !== null) {
			map[status.ID] = statusesIndexedByName['Pending Office of Associate Dean of Research Approval'];
		} else if (stage < 5) {
			map[status.ID] = statusesIndexedByName['Approved'];
		} else {
			map[status.ID] = -1;
		}
		return map;
...
export const getProjectsRevisionStatusId = project => {
	return caesCache.data.crfp.status.map(status => {
		if (status.NAME === 'Pending Superintendent Approval') {
			return statusesIndexedByName['Pending PI Review of Superintendent Comments'];
		} else if (status.NAME === 'Pending Department Head Approval') {
			return statusesIndexedByName['Pending PI Review of Department Head Comments'];
		} else if (status.NAME === 'Pending Final Site Approver Approval') {
			return statusesIndexedByName['Pending PI Review of Final Site Approver Comments'];
		} else if (status.NAME === 'Pending Office of Associate Dean of Research Approval') {
			return statusesIndexedByName['Pending PI Review of Office of Associate Dean of Research Comments'];
		} else {
			return status;
		}
